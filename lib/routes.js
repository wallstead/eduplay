			...
			await JSON.stringify(client.query('SELECT id FROM users WHERE email=$1', [req.body.username], (err, result) => {
				if (err) {
					...
					if (result.rows[0]) {
						...
						client.query('INSERT INTO users (id, firstname, lastname, email, password) VALUES ($1, $2, $3, $4, $5)', [uuidv4(), req.body.firstname, req.body.lastname, req.body.username, password], function (err, result) {
							if (err) {
								console.log(err);
							} else {
