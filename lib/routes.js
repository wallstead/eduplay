...
module.exports = function (app) {
	...
	app.get('/my-games/:gameName', async function (req, res) {
		if (req.isAuthenticated()) {
			...
			try {
				await client.query('BEGIN')
				var game = await JSON.stringify(client.query('SELECT name, gametype, options, games.id, code FROM games JOIN teacher_student_bridge ON games.teacherid = teacher_student_bridge.teacherid WHERE teacher_student_bridge.teacherid=$1 AND LOWER(name)=LOWER($2)', [req.user[0].id, gameName], function (err, result) {
