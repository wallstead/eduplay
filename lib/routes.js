...
module.exports = function (app) {
	...
	/* test out the unity game http request */
	app.get('/testgamedata', async function (req, res) {
	...
	app.post('/create-game', async function (req, res) {
		if (req.isAuthenticated()) {
			...
			var gameData = {}
			if (req.body.words) { // spelling game
				gameData.words = req.body.words
			} else if (req.body.problems) { // math game
	...
	app.post('/update-game', async function (req, res) {
		if (req.isAuthenticated()) {
			var gameData = {}
			if (req.body.words.length > 0) { // spelling game
				gameData.words = req.body.words
			} else if (req.body.problems.length > 0) { // math game
				gameData.problems = req.body.problems
				if (gameData.problems[0].includes("+")) { // addition

				} else if (gameData.problems[0].includes("-")) {
			...
			try {
				const client = await pool.connect()
				await client.query('BEGIN')
				await client.query('UPDATE games SET options = $1 WHERE id = $2', [gameData, req.body.id], function (err, result) {
					if (err) {
						console.log(err);
					} else {
					...
					if (err) {
						...
						client.query('SELECT * FROM games_played WHERE studentid=$1 AND gameid=$2', [req.user[0].id, gameid], function (err, result) {
							if (err) {
								...
								// console.log(result.rows[0].body.results)

								/* show different template depending on game type */
								// spelling first
								// console.log(result.rows[result.rows.length - 1].results.results)
								var jsonData = JSON.parse(result.rows[result.rows.length - 1].results.results);
								console.log(jsonData)
								console.log(jsonData.words)
