...
module.exports = function (app) {
	...
	app.get('/my-games/:gameName', async function (req, res) {
		if (req.isAuthenticated()) {
			...
			try {
				await client.query('BEGIN')
				var game = await JSON.stringify(client.query('SELECT * FROM games WHERE LOWER(name)=LOWER($1) AND teacherid=$2', [gameName, req.user[0].id], function (err, result) {
	...
	app.get('/games/:gameName', async function (req, res) {
		if (req.isAuthenticated()) {
			...
			try {
				await client.query('BEGIN')
				var game = await JSON.stringify(client.query('SELECT name, gametype, options FROM games JOIN teacher_student_bridge ON games.teacherid = teacher_student_bridge.teacherid WHERE teacher_student_bridge.studentid=$1 AND LOWER(name)=LOWER($2)', [req.user[0].id, gameName], function (err, result) {
